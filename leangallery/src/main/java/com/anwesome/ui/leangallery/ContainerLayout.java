...
public class ContainerLayout extends ViewGroup {
    ...
    public void onLayout(boolean reloaded,int a,int b,int w,int h) {
        for(int i=0;i<getChildCount();i++) {
            ...
            else if(child instanceof LeanGalleryLayout) {
                leanGalleryLayout.layout(0,3*h/4,child.getMeasuredWidth(),h);
            }
        }
    }
    ...
    public void addViews() {
        ...
        addView(leanGalleryLayout,new LayoutParams(LayoutParams.WRAP_CONTENT,LayoutParams.WRAP_CONTENT));
    }
